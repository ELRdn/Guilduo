...
## 15. Marketing Landing Page v2 `/lpv2/`

`/lpv2/`と`/lpv2/en/`は既存LPと並存する独立Surface。仕様は[`LPDESIGN.md`](LPDESIGN.md) v2に従う。既存LP・本体・共有パレットを変更しない。

- Human → Agent、Agent → Human、テキストFBを伴う操作体験を中心にする。公式コピーと既存のLPトークンを継承する。
- Guilduoの簡略デモ領域はタスク本文・担当・進捗・FBを表示する。成果物は「外部の作業画面」と明示した別領域で表示する。
- デモはブラウザ内の合成データのみで動く。現在の製品機能と、後続の本体・Skillsで目指す体験を分けて説明する。
- 依頼・通知・担当交代・外部画面の変化・共同完了に短いmotionを使い、Reduced Motionでも同じ操作と結果を提供する。
- レイアウトとスタイルはLPv2に閉じる。古い製品画像を使わず、HTML/CSS/SVGで体験用UIを構成する。

## 16. Marketing Landing Page v2.1 `/lpv2-1/`

`/lpv2-1/`と`/lpv2-1/en/`はLPv2に動きを追加した比較用Surface。LPv2のソースは変更せず、体験ロジックと基礎CSSを読み込み、追加演出を独立実装する。コピー、パレット、正式エンブレム、テキストと外部成果物の境界を維持する。

- HeroのHuman → Agent／Agent → Humanを、中央のGuilduoを通る上下二本の直線で示す。交差や∞型の軌道を使わず、往路と復路のタスク移動をループする。
- `H + A = ✓`は各記号の位相をずらし、最大8pxの上下移動と2度程度の揺れで浮遊させる。MCPと担当交代の方向図もゆったりループする。各図は画面外で一時停止し、再表示時に続きから再開する。
- 見出しは480〜680ms、本文・段階表示は360〜520msで一度だけ現れる。移動量は24px以内、短いease-out、bounceなし。上記9節のアプリ用時間より長い演出を、このLPだけに許可する。
- デモは状態に合わせて送信・受信・完了を強調する。ページのスクロールを乗っ取らず、入力を待たせず、外部画面をGuilduo内へ移動する演出は使わない。
- Reduced Motion、モーションOFF、非表示タブでは移動・点滅・拡縮を停止する。JavaScript無効時も本文・静止した往復図・初期展開された接続説明を読める。
- 詳細な意図と確認範囲は`lpv2-1/README.md`に記録する。
