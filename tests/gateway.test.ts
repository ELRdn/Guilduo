...
test("MCP advertises quest, social, battle, and Toggl Focus tools and calls the same REST domain", async () => {
  ...
  assert.equal(tools.result.tools.length, 56);
  assert.ok(tools.result.tools.some((tool) => tool.name === "request_human_review"));
  assert.ok(tools.result.tools.some((tool) => tool.name === "list_human_requests"));
...
test("MCP v2.7 SDK lane exposes Agent resources, Focus resources, and workflow prompts", async () => {
  ...
  assert.equal(tools.tools.length, 56);
  assert.equal(tools.tools.filter((tool) => tool.outputSchema).length, 56);
