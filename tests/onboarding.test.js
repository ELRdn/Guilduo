...
test("guest starts without developer To Do samples and can add the first quest", () => {
  ...
  const japaneseCatalog = fs.readFileSync(path.join(root, "locales/ja.mjs"), "utf8");
...
test("integration onboarding exposes four steps and provider setup status", () => {
  ...
  const app = fs.readFileSync(path.join(root, "app.js"), "utf8");
  const japaneseCatalog = fs.readFileSync(path.join(root, "locales/ja.mjs"), "utf8");
  const integrations = fs.readFileSync(path.join(root, "worker/src/integrations.mjs"), "utf8");
...
test("battle cards explicitly support quests without notes", () => {
  const app = fs.readFileSync(path.join(root, "app.js"), "utf8");
  const core = fs.readFileSync(path.join(root, "questforge-core.js"), "utf8");
  const japaneseCatalog = fs.readFileSync(path.join(root, "locales/ja.mjs"), "utf8");
