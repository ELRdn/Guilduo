...
test.before(async () => {
  social = await import("../worker/src/social-store.mjs");
});

test.beforeEach(() => social?.resetSocialMemoryForTests());

async function profile(env, uid, handle = uid) {
  return social.upsertProfile(env, uid, {
...
test("handles normalize, validate, reject reserved values, and stay isolated by env", async () => {
  ...
  assert.throws(() => social.validateHandle("ab"), (error) => error.code === "handle_invalid");
  assert.throws(() => social.validateHandle("admin"), (error) => error.code === "handle_reserved");
...
test("profiles enforce uniqueness and expose only approved public fields", async () => {
  ...
  const found = await social.findProfileByHandle(env, "@ALPHA_USER");
...
test("profiles accept a bounded image data URL and reject unsafe avatar values", async () => {
  ...
  const saved = await social.upsertProfile(env, "avatar-user", { displayName: "Avatar User", handle: "avatar_user", avatarUrl });
...
test("handle changes have a 30 day cooldown", async () => {
  const env = { SOCIAL_NOW: "2026-01-01T00:00:00.000Z" };
  ...
  const changed = await social.upsertProfile(env, "alpha", { handle: "alpha_two" });
...
test("party invites persist only a hash and can be accepted once", async () => {
  ...
  const party = await social.acceptPartyInvite(env, "member", { token: issued.token });
...
test("a user can belong to one party and only owners can manage members", async () => {
  ...
  const party = await social.removePartyMember(env, "owner", "member");
...
test("parties cap membership at four", async () => {
  ...
  assert.equal((await social.getParty(env, "owner")).members.length, 4);
...
test("owner departure transfers ownership to the earliest joined member", async () => {
  const env = { SOCIAL_NOW: "2026-03-01T00:00:00.000Z" };
  ...
  const party = await social.getParty(env, "second");
...
test("party invites expire after seven days", async () => {
  const env = { SOCIAL_NOW: "2026-04-01T00:00:00.000Z" };
  await profile(env, "owner", "owner_user");
  await profile(env, "member", "member_user");
...
test("an empty owner party is removed when the owner leaves", async () => {
  const env = {};
  await profile(env, "owner", "owner_user");
  const party = await social.createParty(env, "owner", { name: "Solo" });
  const result = await social.leaveParty(env, "owner");
