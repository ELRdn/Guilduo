...
test("integration vault encrypts provider tokens and isolates users", async () => {
  const { decryptSecret, encryptSecret, getIntegrationAccount, saveIntegrationAccount } = await import("../worker/src/integration-store.ts");
...
test("provider connect creates a short-lived Google OAuth request with minimum scopes", async () => {
  ...
  const callback = await (await import("../worker/src/provider-oauth.ts")).handleProviderCallback(new Request("https://worker.example/oauth/callback/google?state=changed&code=nope"), env, "google");
...
test("Calendar sync caches schedule blocks without creating quests", async () => {
  ...
  const { calendarSchedule, syncIntegration } = await import("../worker/src/integrations.ts");
...
test("Google Tasks conflict stays pending until the user chooses a side", async () => {
  const { saveIntegrationAccount } = await import("../worker/src/integration-store.ts");
  const { resolveGoogleTaskConflict, syncIntegration } = await import("../worker/src/integrations.ts");
