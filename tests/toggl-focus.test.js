...
test("Toggl Focus API keys stay encrypted and never appear in the public connection result", async () => {
  const { connectTogglFocus } = await import("../worker/src/toggl-focus.mjs");
  const { getIntegrationAccount } = await import("../worker/src/integration-store.mjs");
...
test("Focus task creation is idempotent across a Firebase retry and keeps completed To Dos out", async () => {
  const { createQuest, scoreQuest } = await import("../server/questforge-domain.mjs");
  const { syncQuestToTogglFocus } = await import("../worker/src/toggl-focus.mjs");
...
test("Focus timers require the exact current entry before another task can replace or stop it", async () => {
  ...
  const { startTogglFocusTracking, stopTogglFocusTracking } = await import("../worker/src/toggl-focus.mjs");
...
test("Focus time entries are attributed once, override Track totals, and cannot be manually spoofed", async () => {
  const { createQuest, linkExternalRecord } = await import("../server/questforge-domain.mjs");
  const { applyTogglFocusAttribution, previewTogglFocusAttribution } = await import("../worker/src/toggl-focus.mjs");
  const { saveTogglFocusAttribution } = await import("../worker/src/integration-store.mjs");
