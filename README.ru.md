...
| Ошибка разрешения `agents:write` | Повторите авторизацию подключения и подтвердите разрешение управления связями Agent (`agents:write`) на экране согласия. Существующие OAuth-гранты никогда не расширяются автоматически. |
